...
# H3 and ComfyUI use the current Kaggle Python environment. The runtime
# versions remain authoritative in configs/runtime_versions.yaml; worker
# processes are still launched as fresh subprocesses so CUDA/Torch state is
# evaluated independently of any modules already imported in the notebook.
H3_TORCH_CONSTRAINTS = Path("/tmp") / f"minimax_h3_torch_constraints_{os.getpid()}.txt"
...
def _h3_runtime_python() -> Path:
    """Return the current Kaggle Python used by H3 worker subprocesses."""
    ...
    if not python.is_file() or not os.access(python, os.X_OK):
        raise RuntimeError(f"H3 worker Python is missing or not executable: {python}")
    return python


def _h3_runtime_site_packages() -> list[Path]:
    """Resolve site-packages from the same interpreter used by H3 workers."""
    ...
    return [Path(line.strip()) for line in probe.stdout.splitlines() if line.strip()]


def _h3_cuda_library_dirs() -> list[Path]:
    """Resolve Torch/NVIDIA shared-library directories from the current Python."""
    directories: list[Path] = []
    for site_root in _h3_runtime_site_packages():
        torch_lib = site_root / "torch" / "lib"
        if torch_lib.is_dir() and torch_lib not in directories:
            directories.append(torch_lib)
        nvidia_root = site_root / "nvidia"
        if not nvidia_root.is_dir():
            continue
        for directory in sorted(
            (path for path in nvidia_root.rglob("lib") if path.is_dir()),
...
def _h3_runtime_environment(base_env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    if not library_dirs:
        raise RuntimeError(
            "Current H3 worker Python has no Torch/NVIDIA library directories: "
            f"{_h3_runtime_python()}"
        )
    ...
    environment["H3_RUNTIME_ENV_DIR"] = str(Path(sys.prefix).resolve())
    environment["H3_RUNTIME_LIBRARY_PATH"] = ":".join(str(path) for path in library_dirs)
    return environment


def ensure_h3_runtime_python(runtime: dict) -> Path:
    """Validate that the live Kaggle interpreter matches the manifest Python version."""
    python_lock = str(runtime.get("python", {}).get("kaggle", "") or "").strip()
    if not python_lock:
        raise RuntimeError("runtime_versions.yaml python.kaggle is required.")
    ...
    if actual != expected_major_minor:
        raise RuntimeError(
            "Current Kaggle Python does not match runtime_versions.yaml: "
            f"expected={expected_major_minor}, actual={actual}"
        )
    python = _h3_runtime_python()
...
def _write_h3_torch_constraints(expected_torch: str, expected_tv: str, expected_ta: str) -> Path:
    """Write derived Torch constraints for downstream dependency installers."""
...
def _cuda_version_from_tag(cuda_tag: str) -> str:
    """Convert a CUDA wheel tag such as cu130 to the Torch CUDA version string."""
    tag = str(cuda_tag or "").strip().lower()
    if not tag.startswith("cu") or not tag[2:].isdigit() or len(tag[2:]) < 3:
        raise RuntimeError(f"Unsupported CUDA wheel tag in runtime_versions.yaml: {cuda_tag!r}")
    digits = tag[2:]
    return f"{int(digits[:-1])}.{int(digits[-1])}"


def install_pytorch_runtime(runtime: dict) -> None:
    """Install the locked Torch trio into the current Kaggle Python environment."""
    ...
    python = ensure_h3_runtime_python(runtime)
    expected_torch = f"{version}+{cuda}"
    expected_cuda = _cuda_version_from_tag(cuda)
    expected_tv = f"{_base_distribution_version(torchvision_version)}+{cuda}"
    expected_ta = f"{_base_distribution_version(torchaudio_version)}+{cuda}"
    _write_h3_torch_constraints(expected_torch, expected_tv, expected_ta)
    ...
    if loaded_torch is not None:
        print(
            ...
            f"fresh H3 workers will use {expected_torch} / CUDA {expected_cuda}."
        )
    ...
    library_dirs = _h3_cuda_library_dirs()
    if not library_dirs:
        raise RuntimeError("Current Python has no CUDA/Torch library directories after Torch installation.")
    os.environ["H3_RUNTIME_PYTHON"] = str(python)
    os.environ["H3_RUNTIME_ENV_DIR"] = str(Path(sys.prefix).resolve())
    ...
    os.environ["LD_LIBRARY_PATH"] = os.pathsep.join(
        [*map(str, library_dirs), inherited_ld] if inherited_ld else list(map(str, library_dirs))
    )
...
expected_tv = os.environ["H3_EXPECTED_TV"]
expected_ta = os.environ["H3_EXPECTED_TA"]
actual_torch = str(torch.__version__)
actual_cuda = str(torch.version.cuda)
if actual_torch != expected_torch:
    raise RuntimeError(f"Fresh Torch mismatch: expected={expected_torch}, actual={actual_torch}")
if actual_cuda != expected_cuda:
    raise RuntimeError(f"Fresh CUDA mismatch: expected={expected_cuda}, actual={actual_cuda}")
if not torch.cuda.is_available():
    raise RuntimeError("Fresh Torch reports CUDA unavailable.")
observed_tv = metadata.version("torchvision")
observed_ta = metadata.version("torchaudio")
if observed_tv != expected_tv:
    raise RuntimeError(f"Fresh torchvision mismatch: expected={expected_tv}, actual={observed_tv}")
if observed_ta != expected_ta:
    raise RuntimeError(f"Fresh torchaudio mismatch: expected={expected_ta}, actual={observed_ta}")
print(f"[PYTORCH] torch={actual_torch} cuda={actual_cuda} gpu_count={torch.cuda.device_count()}: PASS")
print(f"[PYTORCH] torchvision={observed_tv} torchaudio={observed_ta}: PASS")
"""
    environment = _h3_runtime_environment()
    environment.update({
        "H3_EXPECTED_TORCH": expected_torch,
        "H3_EXPECTED_CUDA": expected_cuda,
        "H3_EXPECTED_TV": expected_tv,
    ...
    if verification.returncode != 0:
        raise RuntimeError(
            "Fresh PyTorch runtime verification failed.\n"
...
def install_director_runtime(
    ...
    print("[DIRECTOR]", f"runtime_env={env_dir}")
    ...
    if uv is None:
        ...
        uv = shutil.which("uv")
    if uv is None:
        raise RuntimeError("uv is required for the pinned vLLM source build.")

    # The Director process keeps the manifest's environment path because the
    # existing launcher reads it from planner.config. The environment itself is
    # deliberately thin: it shares the current Kaggle site-packages, including
    # the locked Torch/CUDA runtime, instead of installing another Torch stack.
    env_dir.parent.mkdir(parents=True, exist_ok=True)
    env_cfg = env_dir / "pyvenv.cfg"
    if env_dir.exists() and env_cfg.is_file():
        cfg = env_cfg.read_text(encoding="utf-8").lower()
        if "include-system-site-packages = true" not in cfg:
            print("[DIRECTOR] replacing old isolated environment that duplicated site-packages")
            shutil.rmtree(env_dir)
    elif env_dir.exists():
        print("[DIRECTOR] replacing incomplete Director environment")
        shutil.rmtree(env_dir)

    if not (env_dir / "bin" / "python").is_file():
        # --without-pip avoids ensurepip issues seen on Kaggle while uv can
        # install packages into the environment directly.
        run(
    ...
    # Verify that the thin venv actually resolves the same Torch installation
    # as the Kaggle interpreter before compiling vLLM.
    torch_probe = subprocess.run(
        [
            str(venv_python), "-c",
            "import torch, sys; print(sys.prefix); print(torch.__version__); print(torch.version.cuda); print(torch.__file__)",
        ],
    ...
    if torch_probe.returncode != 0:
        raise RuntimeError(
            "Director environment cannot import the shared Torch runtime.\n"
            + (torch_probe.stdout or "")
            + (torch_probe.stderr or "")
        )
    print("[DIRECTOR] shared Torch probe:", (torch_probe.stdout or "").strip().replace("\n", " | "))

    source_dir = env_dir.parent / f"{env_dir.name}_source"
    repository = "https://github.com/vllm-project/vllm.git"
    tag = f"v{vllm_version}"
    if source_dir.exists():
        if not (source_dir / ".git").is_dir():
            shutil.rmtree(source_dir)
            run("git", "clone", "--depth", "1", "--branch", tag, repository, source_dir)
        else:
            run("git", "-C", source_dir, "fetch", "--tags", "--force", "origin", tag)
            run("git", "-C", source_dir, "reset", "--hard")
            run("git", "-C", source_dir, "clean", "-fdx")
            run("git", "-C", source_dir, "checkout", "--detach", tag)
    else:
        run("git", "clone", "--depth", "1", "--branch", tag, repository, source_dir)

    # vLLM's documented existing-Torch workflow must run from inside its source
    # tree because use_existing_torch.py edits relative requirements paths.
    use_existing = source_dir / "use_existing_torch.py"
    build_requirements = source_dir / "requirements" / "build" / "cuda.txt"
    pyproject = source_dir / "pyproject.toml"
    if not use_existing.is_file() or not build_requirements.is_file() or not pyproject.is_file():
        raise RuntimeError(f"Pinned vLLM source tree is incomplete: {source_dir}")

    build_env = _h3_runtime_environment()
    build_env["UV_LINK_MODE"] = "copy"
    print("[DIRECTOR] preparing existing-Torch vLLM source tree:", source_dir)
    run(venv_python, str(use_existing), cwd=source_dir, env=build_env)
    run(
    ...
    if verification.returncode != 0:
        raise RuntimeError(
            "vLLM existing-Torch source build verification failed.\n"
            + (verification.stdout or "")
            + (verification.stderr or "")
        )
...
ui.storyboard_gradio._ensure_pillow_runtime_before_queue_start()
print("Production controller import + Pillow queue guard: PASS")
...
def install_and_verify_pillow_runtime(runtime: dict) -> None:
    ...
    print("[PILLOW] shared current-Python verification: PASS")
...
def install_embedded_context_ir_node(runtime: dict) -> None:
    ...
    The source repository deliberately does not carry a custom_nodes tree; this
    package is materialized only inside the runtime ComfyUI checkout.
    """
    ...
    os.environ["H3_CONTEXT_IR_TIMEOUT_SECONDS"] = str(int(features["context_ir_official_timeout_seconds"]))
    os.environ["H3_CONTEXT_IR_POLL_INTERVAL_SECONDS"] = str(float(features["context_ir_official_poll_interval_seconds"]))
    os.environ["H3_CONTEXT_IR_MAX_POLLS"] = str(int(features["context_ir_official_max_polls"]))
    os.environ["H3_CONTEXT_IR_MAX_IMAGE_MB"] = str(float(features.get("context_ir_official_max_image_mb", 30)))
    os.environ["H3_CONTEXT_IR_MAX_VIDEO_MB"] = str(float(features.get("context_ir_official_max_video_mb", 50)))
    os.environ["H3_CONTEXT_IR_MAX_AUDIO_MB"] = str(float(features.get("context_ir_official_max_audio_mb", 15)))
    (destination / "__init__.py").write_text(EMBEDDED_CONTEXT_IR_NODE, encoding="utf-8")
...
def verify_h3_optimization_runtime(runtime: dict) -> None:
    ...
    pytorch_cuda_tag = str(pytorch_cfg.get("cuda", "") or "").strip().lower()
    expected_torch = f"{pytorch_version}+{pytorch_cuda_tag}"
    expected_cuda = _cuda_version_from_tag(pytorch_cuda_tag)
    environment = _h3_runtime_environment()
    environment.update({
        "H3_EXPECTED_TORCH": expected_torch,
        "H3_EXPECTED_CUDA": expected_cuda,
...
def install_h3_context_ir_dependency() -> None:
    """Install the dependency used by the embedded official Context-IR node."""
    run(
        _h3_runtime_python(),
        "-m",
        "pip",
...
def main():
    ...
    # Remove the obsolete H3 isolation directory from earlier bootstrap
    # generations. The corrected runtime never recreates it.
    ...
    # Install the manifest-locked Torch family into the current Kaggle Python
    # before ComfyUI/custom-node dependency installation. A derived constraints
    # file prevents later dependency installers from replacing that family.
    ...
    install_nodes()
    install_h3_context_ir_dependency()
    ...
    try:
        H3_TORCH_CONSTRAINTS.unlink(missing_ok=True)
    except OSError:
        pass
