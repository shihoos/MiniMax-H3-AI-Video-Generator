...
class ProductionCheckpoint:
    ...
    def save(self, session_id: str, state: dict[str, Any]) -> Path:
        ...
        fd, tmp_name = tempfile.mkstemp(prefix=".checkpoint_", suffix=".tmp", dir=str(path.parent))
        try:
    ...
    def load(self, session_id: str) -> dict[str, Any]:
        ...
        if int(data.get("checkpoint_version", 0)) != self.VERSION:
            raise RuntimeError("Unsupported production checkpoint version.")
        return data
    ...
    def latest_resumable(
        ...
        for sid in self.list_sessions():
            try:
                ...
                continue
            if state.get("status") not in {"running", "interrupted", "failed"}:
                continue
